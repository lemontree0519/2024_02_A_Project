...
public class ItemDetector : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            currentNearbyItem.CollectItem(GetComponent<PlayerInventory>());     //플레이어 인벤토리를 참조하여 아이템 수집
        }
    }
    ...
    private void CheckForItems()
    {
        ...
        CollectibleItem closestItem = null;                         //가장 가까운 아이템이 초기값
        ...
        if (closestItem != currentNearbyItem)        //가장 가까운 아이템이 변경되었을 때 메세지 표시
        {
            ...
            if (currentNearbyItem != null)          //가장 가까운 아이템 업데이트
            {
